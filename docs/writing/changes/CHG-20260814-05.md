...
- Collaborators:
  ...
  - **OpenAI Codex(審議席)**——前四次連線逾時、第五次通;逐條追認 fable 的裁決,
    並在六個宿主 plugin 那一題上**先判 merge-blocking、收到複驗後明確改判**
...
- Recurrence check: **重複** KN-004,本張內犯了**四次**:探針把統計行當違規行、
  把引擎訊息的字面猜成「超過」(實為「高於」)、把「誠實欄驗到 N 個 command 檔」
  報成同步前的舊數字、把六個副本的變動報成「出貨內容變了」卻沒量 delta 的內容。
  KN-004 因此兩度擴寫:適用範圍從「數字」擴到「**任何寫進斷言的字面值**」,
  再加一句「**凡宣稱『變了』,必須附那個 delta 的內容,不只附它的檔名**」。
...
`--self-test` **17 案**,每條斷言都有紅端。其中案 15 值得記:
...
### 這個 bump 造成的連帶,以及兩席為何仍放行

`fiction` 同時被六個子流派 plugin 當隨附 skill 打包,所以 `build_suite` 同步後,
六份 `plugins/fiction-*/skills/fiction/SKILL.md` 副本也跟著變,而那六個 plugin
自己的版號沒動。**我把它報成「A(a) 的同一個病」,並送兩席。**

codex 初判 merge-blocking、要求六支全 bump。fable 量了我漏量的維度:
...
**規則內容一個字都沒動**,源頭 `skills/fiction/SKILL.md` 本身也只改了版號行。
所以這是「版號戳記變了、行為沒動」,不是 A(a) 的「行為變了、版號沒動」——
A(a) 保護的傷害(已安裝使用者漏接實質變更)在此不存在。

codex 收到複驗後改判維持放行。**不選「六支全 bump」的理由**:那會經同一條
三處同步斷言,強迫六份 `skills/fiction-*/SKILL.md` 的戳記也跟動,產出六個
release note 只能寫「內含戳記變了」的空版本——正是 `CHG-20260814-03`
剛擋掉的「無內容版本污染歷史」。

留下的是一個**已知瑕疵而非零瑕疵**:`fiction-flash` 等六個 plugin 的 `1.0.0`
這個號,現在對應兩種 byte 狀態。由後續 CHG 消除,不在本批粉飾。
...
graph LR
  ...
  subgraph 新增的閘
    G["genre_ratio_freeze.py<br/>17 案 self-test"]
  end
  B1 --> G
```
...
1. **批量其餘五支之前**,pilot 必須全綠且經兩席複核——**已解除**:
   fable 親跑核實並列出兩項 merge-blocking(均已修),codex 第五次連線後逐條追認。
   但兩席同時追加一條新前置:**版號解耦 / bump 閘延伸的那張 CHG 必須排在批量之前**,
   否則五次搬遷會重演五次戳記漣漪(見 ACC 的「## 兩席共同指出的真洞」)
...
已驗收 — ACC-20260814-05。十條驗收條件全部以**附指令的原始輸出**滿足,兩席合意放行。
