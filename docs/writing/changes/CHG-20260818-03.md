...
而「建了 lint 並掛進 CI 卻沒挪表」會紅。

**判準的精確表述**（審議席三讀要求，因為我原本的說法涵蓋不到它實測的兩個邊）：
判的是「**該腳本的字面路徑有沒有出現在 `ci_local.sh` 的文本裡**」。

| 情形 | 判 | 性質 |
|---|---|---|
| 註解裡提到完整路徑 | **算** | 誤紅方向，fail-closed；但訊息不得說「正在被跑」 |
| `(cd skills/x && $PY scripts/y.py)` 改寫呼叫形 | **不算** | 假綠 |
| 建了腳本但從未在 `ci_local.sh` 出現 | 不算 | 它還不是一道閘 |

兩個邊今天都不活（`ci_local.sh` 現行風格全是字面 `$PY skills/<x>/scripts/<y>.py`，
而九支「沒有 lint」的 skill 磁碟上都沒有 `scripts/`）。
