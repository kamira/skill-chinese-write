...
- Collaborators:
  - **使用者**——指出兩個孤兒目錄的存在,並定下合併順序(先合 fixture-cross-review 再 rebase)
  - **Claude Fable 5(審議席)**——B-1 深層目錄與 B-2 頂層檔案兩條逃逸路徑、「11 個檔」是過期實測值、`build_suite.py:60-82` 的逃逸邊界分析、第 7 項的最小補丁範圍
  - **OpenAI Codex(審議席)**——判「不通過」並堅持缺口須在本張補程式碼而非文件放行(對席據此改判);同名異型的型別斷言
- Risk: 中(刪除已被 git 追蹤的檔案;新增會擋 CI 的斷言;marketplace 版本 bump)
- Autonomy: auto(DIR-001 涵蓋中風險)
- Commit/PR: e5b19e6 / 8d50e10 / 10aca65 / 3fd1d01 +(分支 claude/zen-jepsen-2e1290,合入 main;PR 未開)
  - **本張 rebase 過兩次,hash 換過兩輪**:①原基底 46331e8 → 合併後的 main 268d404(hash 2534107 / 5d6cc11 作廢);②268d404 → **05f028e**,因為 fixture-cross-review 是以 **squash merge(PR #11)** 進 origin/main 的,268d404 那條線整個不在 mainline 上(兩者**樹完全相同**,`git diff 268d404 05f028e` 為空)。第二次用 `git rebase --onto 05f028e 268d404`,只搬本張的 4 個 commit,零衝突
- Recurrence check: **重複** KN-001 **第九次與第十次**——第九次是被修的對象(斷言只有單向:名冊 → 磁碟);**第十次是本張自己**(補上的第 6 項,範圍窄於它自己寫的使用者故事,由審議席抓到並擋下「記為已知限制」的放行方式)→ 兩次都已回填 KN-001 證據
...
於是它們停在 2026-08-10 建檔當天的狀態,再也沒動過(`git log` 證實:bizdoc 最後一次是 0383778,techdoc 是 67a7486)。與頂層 `skills/` 的單一真相對比,**兩邊的檔全部已分岔**——分岔數隨基底變動,對 rebase 前的 46331e8 實測 **11 檔**、對現基底 268d404 實測 **13 檔**(fixture-cross-review 動過 `skills/` 端的夾具,孤兒沒跟上,缺口反而擴大);其中 `plugins/bizdoc/skills/bizdoc/assets/sample-gov-good.md` 用的是半形標點——正是 zh-style 判為硬性違規的那一類。
...
| `plugins/` 底下的 `.py` 工具檔 | 一併判定 / 排除 | `build_suite.py`、`catalog_check.py` 是檔案不是目錄 | 第 6 項**只走訪目錄**;散檔改由第 7 項以白名單管(初版把它排除掉,是審議席抓到的洞) |
| 第 7 項的判法 | 遞迴比對內容 / 只斷言登記形狀 | `build_suite --check` 的 added/updated/removed 已經管完**已宣告路徑內**的內容 | **只斷言登記形狀**。遞迴比對內容是重複造閘;兩席審議席就這條分工收斂 |
| `__pycache__` | 一併判定 / 各層排除 | `.gitignore` 已忽略它;本輪確實發生過它讓已刪目錄續存 | **各層排除**。它是執行殘留,拿它報紅只會製造假紅;要靠它繞過治理得 `git add -f` |
| marketplace 版本 | 不動 / bump | `catalog_check --since` 規定動到 `plugins/` 就要 bump | **bump**。刪檔也是實質變動。版號不由人挑,由 `catalog_check --bump <base>` 對 base 現況推導:rebase 前基底 2.2.0 → 2.2.1,rebase 到合併後的 main(2.3.0)後改為 **2.4.0** |
...
flowchart TB
    ...
    B --> D
    B --> E
    E --> F["不被 build_suite 同步<br/>→ 與單一真相分岔<br/>(對 46331e8 為 11 檔、對 268d404 為 13 檔)<br/>(含半形標點的 sample-gov-good.md)"]
...
- [x] T3. marketplace `metadata.version` bump(rebase 後為 2.3.0 → **2.4.0**;版號由 `--bump` 對 base 推導,不由人挑)
  - interfaces: consumes 刪檔這個實質變動 / produces bump 後的 catalog
  - test: `catalog_check --repo . --check` exit 0;`--since origin/main` exit 0;`--bump main` 回報冪等
- [x] T4. `bash .github/ci_local.sh` 全綠
  - interfaces: consumes T1~T3 / produces 全部步驟 exit 0
  - test: exit 0(rebase 前 13 步;rebase 到合併後的 main 之後含 3b/10b/10c/10d 共 17 步)
- [x] T5. **第 7 項:樹形登記**(審議席兩席合意後補——T2 的涵蓋範圍窄於使用者故事 1)
  - interfaces: consumes `scan_plugin_tree` 走訪出的樹 + `PLUGINS` / produces 三類逃逸路徑的問題列表
  - test: `plugins/official/skills/rogue/`、`plugins/official/rogue-dir/`、`plugins/stray-orphan.md` 三個實驗**各自轉紅且訊息點名**;`--self-test` 五條逃逸路徑紅端 + 一條綠端皆可達;`ci_local.sh` 全綠
...
- **pass**:刪除前 exit 1 且訊息點名 bizdoc 與 techdoc;刪除後 exit 0;self-test 綠紅皆可達;六份夾具判定不變;ci_local 全數步驟 exit 0。
