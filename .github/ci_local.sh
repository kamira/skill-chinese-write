...
echo "[1/10] 實際操作驗收(含漂移紅燈可達探針)"
bash .github/verify.sh

echo "[2/10] writing 風格 lint 夾具:好樣本要過、壞樣本要擋"
...
echo "[3/10] fiction lint 夾具:好樣本要過、壞樣本要擋"
...
echo "[4/10] techdoc lint 夾具:兩份 good 要過、bad 在兩種 kind 都要被擋"
...
echo "[5/10] bizdoc lint 夾具:兩份 good 要過、兩份 bad 在對應 kind 要被擋"
...
echo "[10/10] catalog 版本(靜態 + 變動就要 bump)"
