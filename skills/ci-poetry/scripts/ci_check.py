#!/usr/bin/env python3
"""宋詞格律檢查 — 依白香詞譜之體,判韻組與平仄。

## 判什麼、不判什麼

| 判 | 不判(明標) |
|---|---|
| 韻組(`△▲` run)內是否同詞林部 | 未啟用牌 → 「非本工具所收白香體/未判定」 |
| `○●` 位置的平仄(`⊙` 一律放行) | **句數/字數不符 → 未判定**,見下 |
| | 同牌異體(欽定詞譜 2304 體) |
| | 換韻的合法性、通叶「平仄兩組須同部」 |
| | 領字、去聲位、片界 |

**句數或字數對不上,輸出未判定而不是紅。** 白香每調只收一體,欽定詞譜有 2304 體;
對不上最可能是本工具未收的體,不是作者寫錯。3b 實測 31 篇裡的 10 紅,
5 首正是這種(姜夔〈念奴嬌·鬧紅一舸〉、李清照〈念奴嬌〉、辛棄疾 19 句、
晏幾道〈虞美人〉10 句、李之儀〈卜算子〉第 8 句 6 字)。
**把名家的合法變體判成錯,會教人整道閘一起忽略(KN-002)。**
...
# 而使用者稿裡寫 𣘼 會被 `lines_of` 靜默丟字,那句字數就對不上而被列未判定——
...
def check(tune: str, text: str, tbl: dict) -> tuple[list[str], list[str]]:
    ...
    # ── 句數 / 字數不符 → **未判定,不判紅** ──
    # 白香每調只收一體,而欽定詞譜有 2304 體。句式對不上最可能是「本工具未收的體」,
    # 不是作者寫錯。SKILL.md 明文把「同牌異體」列為不判項,引擎判紅就與自己的宣稱衝突。
    # 3b 實測:31 篇裡 10 紅,其中 5 首正是這種——姜夔〈念奴嬌·鬧紅一舸〉逐句全不同、
    # 李清照〈念奴嬌〉、辛棄疾 19 句、晏幾道〈虞美人〉10 句、李之儀〈卜算子〉第 8 句 6 字。
    # **把名家的合法變體判成錯,會教人整道閘一起忽略(KN-002)。**
    spec = tbl["tunes"][tune]["rows"]
    got = lines_of(text)
    if len(got) != len(spec):
        unk.append(f"句數 {len(got)},白香〈{tune}〉之體為 {len(spec)} 句"
                   "——疑為本工具未收之體,未判定;若確為此體請檢查稿件斷句")
        return bad, unk
    off = [f"第 {i} 句 {len(g)} 字(譜為 {len(s['chars'])} 字):「{g}」"
           for i, (g, s) in enumerate(zip(got, spec), 1) if len(g) != len(s["chars"])]
    if off:
        unk.append(f"{len(off)} 句字數與白香〈{tune}〉之體不符——疑為本工具未收之體,"
                   "未判定:" + "、".join(off[:4]) + ("…" if len(off) > 4 else ""))
        return bad, unk
...
def self_test() -> int:
    ...
    # 3 句數不符 → 不硬對齊
    # 3/4 **改判為未判定**(codex 裁):句式對不上最可能是本工具未收的體,
    #     判紅會與 SKILL.md 明文的「同牌異體不判」衝突。
    case("3 句數不符→未判定", "菩薩蠻", "平林漠漠煙如織。", want_unk="疑為本工具未收之體")
    # 4 某句字數不符
    t = tbl["tunes"]["菩薩蠻"]["rows"]
    bads = "，".join((r["chars"] + "多") if i == 0 else r["chars"]
                    for i, r in enumerate(t)) + "。"
    case("4 某句字數不符→未判定", "菩薩蠻", bads, want_unk="第 1 句 8 字")
    # 5 綠端:譜例詞自己過檢
